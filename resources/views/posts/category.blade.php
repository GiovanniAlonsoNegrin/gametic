    ...
    <div class="container py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <h1 class="uppercase text-center text-3xl text-white font-bold mb-4 bg-blue-950 py-4">Etiquetas</h1>
                @forelse ($tags as $tag)
                    <a href="{{ route('posts.tag', $tag) }}">
                        <div class="bg-{{ $tag->color }}-600 my-4 w-6/12 rounded-full py-1 flex justify-center items-center mx-auto group hover:bg-gray-100">
                            <span class="text-bold capitalize text-white text-3xl transition-colors duration-300 group-hover:text-{{ $tag->color }}-600">{{ $tag->name }}</span>
                        </div>
                    </a>
                @empty
                @endforelse
